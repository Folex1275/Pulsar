...
Thank you for your interest in contributing. Pulsar is an open protocol — contributions from the Stellar community are what make it stronger. This document covers everything you need to get involved, from filing your first issue to submitting a production-ready pull request.
...
Pulsar follows the [Contributor Covenant](https://www.contributor-covenant.org/version/2/1/code_of_conduct/) Code of Conduct. By participating you agree to uphold it. Instances of unacceptable behaviour can be reported to the maintainers via the email in the repository's security policy.

---

## Ways to contribute

You do not need to write code to contribute meaningfully to Pulsar.
...
- Write tutorials or integration guides for Pulsar.js
...
| Node.js | `>=18` | Pulsar.js SDK development |
...
```bash
git clone https://github.com/your-org/pulsar
cd pulsar
cargo build
```
...
├── sdk/
│   ├── src/
│   │   ├── client.ts             # PulsarClient (subscriber-facing)
│   │   ├── merchant.ts           # PulsarMerchant (merchant-facing)
...
Pulsar uses a standard fork-and-branch workflow.
...
```bash
git clone https://github.com/YOUR_USERNAME/pulsar
cd pulsar
git remote add upstream https://github.com/your-org/pulsar
```
...
- Use `soroban_sdk::panic_with_error!` with a typed `PulsarError` — never use bare `panic!`
...
All errors must be variants of `PulsarError`:
...
The Pulsar.js SDK lives in `/sdk` and is written in TypeScript with strict mode enabled.
...
2. Implement the method in the appropriate class (`PulsarClient` for subscriber-facing, `PulsarMerchant` for merchant-facing)
...
Soroban contract tests live in `contracts/pulsar/tests/`. Each public function has its own test file. Tests use the Soroban test environment (`soroban_sdk::testutils`) — no deployed contract or network connection is needed.

Every pull request that touches contract logic must include tests for:

- The happy path
- All relevant `PulsarError` variants
