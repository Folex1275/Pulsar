...
Pulsar solves both. A structured `AllowanceRecord` encodes exactly what a merchant may debit, when, how often, and for how many cycles. A permissionless keeper network handles execution. Stellar's native path payments handle multi-asset conversion atomically. No custodian. No cron. No unlimited approvals.
...
At checkout, the subscriber signs a single transaction that calls `create_allowance()` on the Pulsar contract. This stores an `AllowanceRecord` on Soroban — not a token transfer, not an approval — a structured permission record the contract enforces on every billing attempt.
...
Pulsar.js is a lightweight TypeScript SDK for integrating subscription billing into any Web3 application. Drop-in equivalent to Stripe.js for the Soroban ecosystem.

### Installation

```bash
npm install @pulsar-protocol/sdk
```

### Create a subscription (frontend)

```typescript
import { PulsarClient } from '@pulsar-protocol/sdk';

const pulsar = new PulsarClient({ network: 'mainnet' });

const allowanceId = await pulsar.createSubscription({
...
```typescript
import { PulsarMerchant } from '@pulsar-protocol/sdk';

const merchant = new PulsarMerchant({
...
Pulsar is a protocol, not a business. There is no protocol fee on successful billings in v1. The only costs are:
...
**Oracle manipulation** — Not applicable. Pulsar has no oracle dependency. Billing logic is purely ledger-based.
...
### v0.2 — SDK + devtools
- [ ] Pulsar.js SDK (TypeScript)
- [ ] Merchant event webhook relay
- [ ] Keeper monitoring dashboard
- [ ] Local testnet environment (`pulsar-dev`)
...
Pulsar is open source and welcomes contributions. See [CONTRIBUTING.md](./CONTRIBUTING.md) for guidelines.
...
Contract source lives in `/contracts/pulsar`. Keeper bot in `/keeper`. SDK in `/sdk`.
